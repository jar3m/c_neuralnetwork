...
void predict_network(t_neural_network *nwk)
{
	...
	while(fscanf(test_data,"%f%f%f%f%f%f%f%f%f%f", &in[0], &in[1], &in[2], &in[3], &in[4], 
			&in[5], &in[6], &in[7], &in[8], &out[0])!=EOF) {
		feed_forward(nwk, in);
...
int main()
{
	t_layer_info h = {2,{1,1}};
	t_neural_network *nn;
	FILE *train_data;
	float in[9], out[1];

	nn = create_neural_network(9, h, 1);
	...
	while(fscanf(train_data,"%f%f%f%f%f%f%f%f%f%f", &in[0], &in[1], &in[2], &in[3], &in[4], 
			&in[5], &in[6], &in[7], &in[8], &out[0])!=EOF) {
