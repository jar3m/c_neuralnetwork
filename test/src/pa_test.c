...
//	set_bypass_neuron(nn,0,1,1);
//	set_bypass_neuron_op(nn,0,0,0);
//	set_bypass_neuron_op(nn,0,1,1);
	if(nn_config.h.n_layers == 2) {
		set_bypass_neuron(nn,1,0,0);
		set_bypass_neuron(nn,1,1,1);
	}
	for(i = 0; i < nn_config.h.n_layers; i++) {
		set_bypass_neuron(nn,i,0,0);
		set_bypass_neuron(nn,i,1,1);
	}
#endif
